...
class IOGradioUI:
    ...
    def launch(self, share=False):
        ...
        if convos:
            conv_id_input = gr.Dropdown(choices=convos, label="Conversation ID", value=convos[0] if convos else "")
        else:
        ...
        async def agent_chat_fn(history, user_message, conversation_id, css, dynamic_ui_spec, dynamic_ui_values):
            ...
            if user_message is None and dynamic_ui_spec is not None and dynamic_ui_values is not None:
            ...
            result = await agent.run(user_message, conversation_id=conversation_id, pretty=False)
        ...
        with gr.Blocks() as demo:
            ...
            with gr.Row():
                chatbot = gr.Chatbot(label="Chat", elem_id="chatbot", show_copy_button=True, height=600, type="messages")
                ...
                if ui_spec:
                    for comp in ui_spec:
                        if comp["type"] == "textbox":
                            tb = gr.Textbox(label=comp.get("label", ""), value=comp.get("value", ""))
                    ...
                    submit_btn = gr.Button("Submit Dynamic UI", elem_classes=["io-chat-btn"])
                    ...
                    def on_dynamic_submit(*values):
                        ...
                        return agent_chat_fn(
                            chatbot.value, None, conv_id_state.value, css_html.value, ui_spec, list(values)
                        )
                    submit_btn.click(on_dynamic_submit, inputs=dynamic_ui_inputs, outputs=[chatbot, user_input, conv_id_state, css_html, dynamic_ui_spec_state, dynamic_ui_values_state])
                else:
            ...
            demo.load(get_initial_states, inputs=None, outputs=[chatbot, conv_id_state, css_html, dynamic_ui_spec_state, dynamic_ui_values_state])
            ...
            send_btn.click(
                agent_chat_fn,
                inputs=[chatbot, user_input, conv_id_state, css_html, dynamic_ui_spec_state, dynamic_ui_values_state],
                outputs=[chatbot, user_input, conv_id_state, css_html, dynamic_ui_spec_state, dynamic_ui_values_state],
            )
            user_input.submit(
                agent_chat_fn,
                inputs=[chatbot, user_input, conv_id_state, css_html, dynamic_ui_spec_state, dynamic_ui_values_state],
