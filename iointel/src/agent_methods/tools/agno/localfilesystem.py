...
class LocalFileSystem(make_base(AgnoLocalFileSystemTools)):
    def _get_tool(self):
        return self.Inner(
            target_directory=self.target_directory_,
            default_extension=self.default_extension_,
        )
    ...
    def write_file(
        ...
        return self.write_file(self, content, filename, directory, extension)
    ...
    def read_file(self, filename: str, directory: Optional[str] = None) -> str:
        return self.read_file(self, filename, directory)
