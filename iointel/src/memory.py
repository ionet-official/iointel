...
class Memory:
    def __init__(self, connection_string: str):
        ...
        :param connection_string: A SQLAlchemy-compatible database URL.
            For SQLite: "sqlite:///path/to/db.sqlite3"
    ...
    def store_run_history(self, conversation_id: str, result) -> None:
        ...
        try:
            ...
            existing_messages_raw = (
                existing_conversation.messages_json if existing_conversation else "[]"
            )
    ...
    def get_history(self, conversation_id: str) -> str:
        """
        Retrieve stored conversation history as a JSON string for a given conversation_id.
        """
        session = self.SessionLocal()
        try:
            conversation = (
    ...
    def get_message_history(self, conversation_id: str, MAX_MESSAGES=100):
        raw = self.get_history(conversation_id)
        if raw:
            ...
            # Select only the last N messages to stay within token limits
            ...
            for item in filtered_history_list:
                ...
                # Filter out tool-call/tool-return parts explicitly
                filtered_parts = [
                    part
                    for part in parts
